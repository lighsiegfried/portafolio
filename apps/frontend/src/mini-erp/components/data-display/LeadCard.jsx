...
export default function LeadCard({ lead, canManage, onOpen, onMove }) {
  ...
  return (
    <Card
      onClick={() => onOpen(lead)}
      className="cursor-pointer space-y-2 p-3 transition-colors hover:border-violet-500/40 hover:bg-accent"
    >
        ...
        {canManage && (
          <DropdownMenu>
            <DropdownMenuTrigger asChild onClick={(e) => e.stopPropagation()}>
              <button
                type="button"
                aria-label={te.leads.card.moveLead}
                className="rounded-md p-1 text-muted-foreground hover:bg-muted hover:text-foreground"
              >
                <MoreVertical className="size-4" aria-hidden="true" />
                <span className="sr-only">{te.leads.card.moveLead}</span>
              </button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end" onClick={(e) => e.stopPropagation()}>
              <DropdownMenuLabel>{te.leads.card.moveTo}</DropdownMenuLabel>
              <DropdownMenuSeparator />
      ...
      <div className="flex items-center gap-3 text-[11px] text-muted-foreground">
        {lead.nextFollowUp && (
          <span className={cn('inline-flex items-center gap-1', overdue && 'text-red-600 dark:text-red-300')}>
